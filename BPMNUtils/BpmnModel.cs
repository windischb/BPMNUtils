...
namespace BPMNUtils
{
    public class BpmnModel
    {
        ...
        public static BpmnModel FromBytes(byte[] bytes, bool disableValidation = false)
        {
            ...
            {
                results = parser.Parse(stream, disableValidation);
            }

            return new BpmnModel(results.Definitions, results.FlowElements, bytes);
        }

        public static BpmnModel FromStream(Stream stream, bool disableValidation = false)
        {
            using (MemoryStream ms = new MemoryStream())
            {
                stream.CopyTo(ms);
                return FromBytes(ms.ToArray(), disableValidation);
            }
